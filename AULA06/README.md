...
[Extração: BeautifulSoup4 | pdfplumber/PyPDF2 | Tesseract OCR]
        │
        ▼
[Limpeza e normalização: remove ruído, corrige codificação]
        │
        ▼
[Anonimização - regex: nome/transação mascarados]
        │
        ▼
...
[Banco vetorial: indexado com filtro por gateway/status]
        │
        ▼
[Consulta do usuário] → [Retrieval filtrado] → [LLM + contexto] → [Resposta com fonte citada]
        │
        ▼
[Exibida dentro do sistema de gestão do analista]
\```
...
**Respondendo:**
- **Filtrar busca:** `status` + `data_fim` — evita trazer regra de desafio encerrado.
- **Citar fonte:** `title` + `data_inicio`/`data_fim` — ex.: "Fonte: Desafio de Hidratação - Semana 34 (17/08 a 24/08)".
- **Metadado caro de acrescentar depois:** `status` — exigiria reindexar toda a base.
- **Como extrair:** maioria já existe estruturada no banco relacional do app, copiada direto para o metadado vetorial.
...
[Documento fonte: texto de regras digitado no app]
        │
        ▼
[Extração: nenhuma ferramenta especial, já digital]
        │
        ▼
[Limpeza: remove espaços/caracteres invisíveis, garante UTF-8]
        │
        ▼
[Chunking: por parágrafo, sem overlap]
        │
        ▼
...
[Banco vetorial: filtro por status=ativo]
        │
        ▼
[Consulta do usuário] → [Retrieval filtrado] → [LLM + contexto] → [Resposta citando título/período]
        │
        ▼
[Exibida no chat/assistente do app mobile GymRat]
\```
